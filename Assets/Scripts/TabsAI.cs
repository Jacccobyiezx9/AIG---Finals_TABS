...
public class TabsAI : MonoBehaviour
{
    ...
    [SerializeField] UnitCounter unitCounter;
    ...
    private void Start()
    {
        if (myTeam == Team.Human)
            UnitCounter.humanCount++;
        else
            UnitCounter.monsterCount++;
    ...
    private NodeState DetectTarget()
    {
        if (target != null)
        {
            ...
            if (targetAI != null && !targetAI.isDead && distSq <= (loseRange * loseRange))
                return NodeState.Success;
        ...
        int unitLayerMask = LayerMask.GetMask("NPC");
        Collider[] hits = Physics.OverlapSphere(transform.position, detectionRange, unitLayerMask);
        ...
        foreach (var hit in hits)
        {
            ...
            if (hit.TryGetComponent<TabsAI>(out TabsAI ai))
            {
                if (ai.isDead || ai.myTeam != targetTeam) continue;
                float dSq = (transform.position - hit.transform.position).sqrMagnitude;
